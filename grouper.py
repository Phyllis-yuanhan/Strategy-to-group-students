...
class AlphaGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        stu = sort_students(list(course.get_students()), 'name')
        lst = slice_list(stu, self.group_size)
        g = Grouping()
        for i in lst:
            new_group = Group(i)
...
class RandomGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        lst = []
        s = course.get_students()
        while len(lst) != len(s):
            member = s[random.randint(0, len(s) - 1)]
            if member not in lst:
                lst.append(member)
        lst2 = slice_list(lst, self.group_size)
        grouping = Grouping()
        for x in lst2:
            g = Group(x)
...
class GreedyGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        while len(students) != 0:
            if len(g_member) == self.group_size:
                grouping.add_group(Group(g_member))
                g_member = []
            else:
                scores = []
                for s in students:
                    new_group = g_member + [s]
                    score = survey.score_students(new_group)
                    scores.append(score)
                max_score_student = scores.index(max(scores))
                g_member.append(students[max_score_student])
                students.pop(max_score_student)
        grouping.add_group(Group(g_member))
        return grouping
...
class WindowGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        while len(students) >= self.group_size:
            ...
            i = 0
            while i < len(window) - 1 and survey.score_students(window[i]) <\
                    survey.score_students(window[i + 1]):
                i += 1
            if len(window) == i:
                grouping.add_group(Group(window[0]))
            else:
                group = Group(window[i])
...
class Group:
    ...
    def __str__(self) -> str:
        ...
        if len(self._members) != 0:
            an = self._members[0].name
            for i in range(1, len(self._members)):
                an += ',' + self._members[i].name
            return an
        else:
            return ''
...
class Grouping:
    ...
    def __str__(self) -> str:
        ...
        if len(self._groups) != 0:
            an = self._groups[0].__str__()
            for i in range(1, len(self._groups)):
                an += '\n' + self._groups[i].__str__()
            return an
        else:
            return ''
    ...
    def add_group(self, group: Group) -> bool:
        ...
        for s in group.get_members():
            for g in self._groups:
                lst = []
                for x in g.get_members():
                    lst.append(x.id)
                if s.id in lst:
                    return False
