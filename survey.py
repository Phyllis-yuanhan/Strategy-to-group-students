...
class MultipleChoiceQuestion(Question):
    ...
    id: the id of this question
    text: the text of this question
    _options: a list of all of the options
    === Representation Invariants ===
    text is not the empty string
    """
    id: int
    ...
    def __str__(self) -> str:
        ...
        an = self._options[0]
        for i in range(1, len(self._options)):
            an += "," + self._options[i]
        s = 'The question is ' + self.text + \
            ' with the following possible choices ' + an
        return s
...
class NumericQuestion(Question):
    ...
    id: the id of this question
    text: the text of this question
    _min: the minimum value of answers
    _max: the maximum value of answers
    === Representation Invariants ===
    text is not the empty string
    """
    ...
    def __str__(self) -> str:
        ...
        s = 'The question is ' + self.text + ' whose answers are between ' + \
            str(self._min) + ' and ' + str(self._max)
        return s
    ...
    def get_similarity(self, answer1: Answer, answer2: Answer) -> float:
        ...
        ad = abs(answer1.content - answer2.content)
        num = self._max - self._min
        result = 1 - float(ad/num)
        return result
...
class YesNoQuestion(Question):
    ...
    def __str__(self) -> str:
        ...
        s = 'The question is ' + self.text +\
            ' whose answer is either True or False.'
        return s
...
class CheckboxQuestion(Question):
    ...
    id: the id of this question
    text: the text of this question
    _options: a list of all of the answers
    ...
    def validate_answer(self, answer: Answer) -> bool:
        ...
        for i in range(len(answer.content)):
            if answer.content[i] in answer.content[i + 1:]:
                return False
            if answer.content[i] not in self._options:
                return False
        return True
    ...
    def get_similarity(self, answer1: Answer, answer2: Answer) -> float:
        ...
        for i in answer1.content:
            unique.append(i)
        common = 0
        for a in answer2.content:
            if a not in unique:
                unique.append(a)
            else:
                common += 1
        return common / len(unique)
...
class Survey:
    ...
    def score_students(self, students: List[Student]) -> float:
        ...
        for i in self._questions:
            q = self._questions[i]
            c = self._get_criterion(q)
            w = self._get_weight(q)
            ans = []
            for s in students:
                ans.append(s.get_answer(q))
            try:
                score += c.score_answers(q, ans) * w
    ...
    def score_grouping(self, grouping: Grouping) -> float:
        ...
        for g in grouping.get_groups():
            lst = g.get_members()
            s += self.score_students(lst)
        return s / len(grouping.get_groups())
